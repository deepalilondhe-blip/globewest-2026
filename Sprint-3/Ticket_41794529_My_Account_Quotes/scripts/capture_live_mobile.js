...
const OUT_DIR = '/home/deepali/My Projects/Deepali/GlobeWest 2026 (2)/Sprint-3/Ticket_41794529_My_Account_Quotes/screenshots/mobile';
if (!fs.existsSync(OUT_DIR)) fs.mkdirSync(OUT_DIR, { recursive: true });

(async () => {
  console.log('🚀 Capturing Mobile Live Staging for My Quotes...');
  ...
  try {
    ...
    const fullPath = path.join(OUT_DIR, '01_my_quotes_mobile_live_full.png');
    await page.screenshot({ path: fullPath, fullPage: true });
    console.log('📸 Saved Mobile Full Page:', fullPath);

    // Viewport screenshot
    const vpPath = path.join(OUT_DIR, '02_my_quotes_mobile_live_viewport.png');
    ...
    if (await tableEl.isVisible().catch(() => false)) {
      const tablePath = path.join(OUT_DIR, '03_my_quotes_mobile_table_area.png');
