#!/usr/bin/env python3
"""
Generate comprehensive Test Cases Excel and CSV for Ticket #41794530: My Account - Orders.
"""
...
# Write CSV
csv_path = os.path.join(BASE_DIR, 'My_Orders_Test_Cases.csv')
...
# Write XLSX with OpenPyXL styling
xlsx_path = os.path.join(BASE_DIR, 'My_Orders_Test_Cases.xlsx')
df = pd.DataFrame(test_cases)
with pd.ExcelWriter(xlsx_path, engine='openpyxl') as writer:
    df.to_excel(writer, sheet_name='My Orders Test Cases', index=False)
    
    # Format worksheet
    ws = writer.sheets['My Orders Test Cases']
    ...
    for row in ws.iter_rows(min_row=2, max_row=len(test_cases)+1):
        status_val = row[7].value # Status column (0-indexed 7)
        for cell in row:
        ...
        elif "CRITICAL" in str(status_val):
            status_cell.fill = PatternFill(start_color="FEE2E2", end_color="FEE2E2", fill_type="solid")
            status_cell.font = Font(name="Calibri", size=10, bold=True, color="7F1D1D")
        else:
            status_cell.fill = PatternFill(start_color="FEF3C7", end_color="FEF3C7", fill_type="solid")
            status_cell.font = Font(name="Calibri", size=10, bold=True, color="92400E")
    ...
    col_widths = {
        ...
        'I': 22, # Status
        'J': 25, # Severity
        'K': 35  # Evidence Reference
    }
    for col, width in col_widths.items():
        ws.column_dimensions[col].width = width

print(f"✅ Generated Styled Excel: {xlsx_path}")
